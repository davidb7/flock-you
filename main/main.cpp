...
// ============================================================================
// ESP-IDF port — migrated from Arduino/PlatformIO.
// All detection logic is identical to the Arduino version in src/main.cpp.
//
// API mapping summary:
//   millis()            -> esp_timer_get_time() / 1000 (shim below)
//   delay(N)            -> vTaskDelay(pdMS_TO_TICKS(N)) (shim below)
//   tone()/noTone()     -> LEDC peripheral
//   WiFi.softAP()       -> esp_wifi_* APIs
//   SPIFFS              -> esp_vfs_spiffs + standard C FILE*
//   AsyncWebServer      -> esp_http_server (chunked responses)
//   Serial              -> USB CDC via stdin/stdout (CONFIG_ESP_CONSOLE_USB_CDC)
//   setup()/loop()      -> app_main() + FreeRTOS task
//   NimBLE              -> h2zero/esp-nimble-cpp (same C++ API as NimBLE-Arduino)
// ============================================================================
...
// BLE — h2zero/esp-nimble-cpp (same API as NimBLE-Arduino 1.4.x)
...
// delay() is used throughout audio functions which run in the main task.
// vTaskDelay satisfies this; min resolution is 1ms with CONFIG_FREERTOS_HZ=1000.
...
static const uint16_t ble_manufacturer_ids[] = {
    0x09C8   // XUNTONG — source: wgreenberg/flock-you
};
...
static httpd_handle_t fyHttpd = NULL;
...
static volatile bool         fySerialByteReceived  = false;
...
// the main loop applies WiFi/scan changes in its own task context.
...
#define FY_SESSION_FILE  SPIFFS_BASE "/session.json"
#define FY_PREV_FILE     SPIFFS_BASE "/prev_session.json"
...
// WiFi netif handle (needed to stop/start AP in companion mode)
static esp_netif_t* fyApNetif = NULL;

// ============================================================================
// LEDC / AUDIO
// ============================================================================

#define FY_LEDC_MODE       LEDC_LOW_SPEED_MODE
#define FY_LEDC_TIMER      LEDC_TIMER_0
#define FY_LEDC_CHANNEL    LEDC_CHANNEL_0
#define FY_LEDC_RESOLUTION LEDC_TIMER_10_BIT
#define FY_LEDC_DUTY_50PCT 512u  // 50% of 2^10

static void fyLedcInit() {
    ledc_timer_config_t t = {};
    t.speed_mode      = FY_LEDC_MODE;
    t.duty_resolution = FY_LEDC_RESOLUTION;
    t.timer_num       = FY_LEDC_TIMER;
    t.freq_hz         = 1000;
    ...
    ch.speed_mode = FY_LEDC_MODE;
    ch.channel    = FY_LEDC_CHANNEL;
    ch.intr_type  = LEDC_INTR_DISABLE;
    ch.timer_sel  = FY_LEDC_TIMER;
    ch.duty       = 0;
    ch.hpoint     = 0;
    ledc_channel_config(&ch);
}

// Drop-in replacements for Arduino tone()/noTone().
// The duration parameter in fyCaw() is not used for auto-stop (the original
// loop calls tone() every 8ms overriding it), so we ignore it here too.
static void tone(gpio_num_t /*pin*/, int freq, int /*dur_ms*/) {
    ledc_set_freq(FY_LEDC_MODE, FY_LEDC_TIMER, freq);
    ledc_set_duty(FY_LEDC_MODE, FY_LEDC_CHANNEL, FY_LEDC_DUTY_50PCT);
    ledc_update_duty(FY_LEDC_MODE, FY_LEDC_CHANNEL);
}

static void noTone(gpio_num_t /*pin*/) {
    ledc_set_duty(FY_LEDC_MODE, FY_LEDC_CHANNEL, 0);
    ledc_update_duty(FY_LEDC_MODE, FY_LEDC_CHANNEL);
}
...
// Crow caw: descending frequency sweep with warble texture
...
static void fyDetectBeep() {
    ...
    if (!fyBuzzerOn) return;
    fyCaw(400, 900, 100, 30); delay(60);
    fyCaw(450, 950, 100, 30); delay(60);
...
static void fySendBLE(const char* data, size_t len) {
    ...
    if (len <= chunkSize) {
        ...
        size_t offset = 0;
        while (offset < len) {
            size_t send = (len - offset) < chunkSize ? (len - offset) : chunkSize;
...
static void fyWiFiInit() {
    ESP_ERROR_CHECK(esp_netif_init());
    ESP_ERROR_CHECK(esp_event_loop_create_default());
    fyApNetif = esp_netif_create_default_wifi_ap();
    ...
    strlcpy((char*)ap_config.ap.ssid,     FY_AP_SSID, sizeof(ap_config.ap.ssid));
    strlcpy((char*)ap_config.ap.password, FY_AP_PASS, sizeof(ap_config.ap.password));
    ap_config.ap.ssid_len      = strlen(FY_AP_SSID);
...
// Called from the main loop — never from a BLE callback context.
static void fyOnCompanionChange() {
    if (fyBLEClientConnected || fySerialHostConnected) {
        fyStopWiFiAP();
        fyBleScanDuration = 3;
        printf("[FLOCK-YOU] Companion mode: WiFi AP OFF, scan %ds\n", fyBleScanDuration);
    } else {
        fyStartWiFiAP();
        fyBleScanDuration = 2;
        printf("[FLOCK-YOU] Standalone mode: WiFi AP ON, scan %ds\n", fyBleScanDuration);
    }
}
...
// ============================================================================
// SESSION PERSISTENCE (SPIFFS via standard C file I/O)
// ============================================================================

static bool fyInitSpiffs() {
    ...
    if (ret != ESP_OK) {
        printf("[FLOCK-YOU] SPIFFS init failed: %s\n", esp_err_to_name(ret));
        return false;
    }
...
static void fySaveSession() {
    ...
    FILE* f = fopen(FY_SESSION_FILE, "w");
    ...
    for (int i = 0; i < fyDetCount; i++) {
        ...
        fprintf(f, "{\"mac\":\"%s\",\"name\":\"%s\",\"rssi\":%d,\"method\":\"%s\","
        ...
        if (d.hasGPS) {
            fprintf(f, ",\"gps\":{\"lat\":%.8f,\"lon\":%.8f,\"acc\":%.1f}",
...
static void fyPromotePrevSession() {
    ...
    FILE* src = fopen(FY_SESSION_FILE, "r");
    if (!src) {
    ...
    remove(FY_SESSION_FILE);
    printf("[FLOCK-YOU] Prior session promoted: %ld bytes\n", size);
}

// ============================================================================
// HTTP SERVER — CHUNKED RESPONSE HELPERS
// ============================================================================

static void resp_chunk(httpd_req_t* req, const char* str) {
    httpd_resp_send_chunk(req, str, strlen(str));
}

static void resp_chunkf(httpd_req_t* req, const char* fmt, ...) {
    char buf[512];
    va_list args;
    va_start(args, fmt);
...
// ============================================================================
// HTTP SERVER — JSON / KML GENERATORS
// ============================================================================

static void writeDetectionsJSON(httpd_req_t* req) {
    resp_chunk(req, "[");
    if (fyMutex && xSemaphoreTake(fyMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
        for (int i = 0; i < fyDetCount; i++) {
            if (i > 0) resp_chunk(req, ",");
            resp_chunkf(req,
            ...
            if (fyDet[i].hasGPS) {
                resp_chunkf(req, ",\"gps\":{\"lat\":%.8f,\"lon\":%.8f,\"acc\":%.1f}",
...
static void writeDetectionsKML(httpd_req_t* req) {
    resp_chunk(req,
    ...
    if (fyMutex && xSemaphoreTake(fyMutex, pdMS_TO_TICKS(300)) == pdTRUE) {
        for (int i = 0; i < fyDetCount; i++) {
            ...
            resp_chunk(req, "<Placemark>\n");
            resp_chunkf(req, "<name>%s</name>\n", d.mac);
            resp_chunkf(req, "<styleUrl>#%s</styleUrl>\n", d.isRaven ? "raven" : "det");
            resp_chunk(req, "<description><![CDATA[");
            if (d.name[0]) resp_chunkf(req, "<b>Name:</b> %s<br/>", d.name);
            resp_chunkf(req, "<b>Method:</b> %s<br/><b>RSSI:</b> %d dBm<br/>"
                             "<b>Count:</b> %d<br/>", d.method, d.rssi, d.count);
            if (d.isRaven) resp_chunkf(req, "<b>Raven FW:</b> %s<br/>", d.ravenFW);
            resp_chunkf(req, "<b>Accuracy:</b> %.1f m", d.gpsAcc);
            resp_chunk(req, "]]></description>\n");
            resp_chunkf(req, "<Point><coordinates>%.8f,%.8f,0</coordinates></Point>\n",
                        d.gpsLon, d.gpsLat);
            resp_chunk(req, "</Placemark>\n");
        }
        xSemaphoreGive(fyMutex);
    }
    resp_chunk(req, "</Document>\n</kml>");
    httpd_resp_send_chunk(req, NULL, 0);
}
...
static const char FY_HTML[] = R"rawliteral(
...
static esp_err_t handler_root(httpd_req_t* req) {
    ...
    httpd_resp_send(req, FY_HTML, strlen(FY_HTML));
    return ESP_OK;
}

static esp_err_t handler_detections(httpd_req_t* req) {
    httpd_resp_set_type(req, "application/json");
    writeDetectionsJSON(req);
    return ESP_OK;
}
...
static esp_err_t handler_stats(httpd_req_t* req) {
    ...
    snprintf(buf, sizeof(buf),
        ...
        fyGPSValid ? (unsigned long)(millis() - fyGPSLastUpdate) : 0UL,
        withGPS);
    httpd_resp_set_type(req, "application/json");
...
static esp_err_t handler_gps(httpd_req_t* req) {
    char query[128] = {};
    char lat_str[32] = {}, lon_str[32] = {}, acc_str[32] = {};

    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
        httpd_query_key_value(query, "lat", lat_str, sizeof(lat_str));
        httpd_query_key_value(query, "lon", lon_str, sizeof(lon_str));
        httpd_query_key_value(query, "acc", acc_str, sizeof(acc_str));
    }

    if (lat_str[0] && lon_str[0]) {
        fyGPSLat        = atof(lat_str);
        fyGPSLon        = atof(lon_str);
        fyGPSAcc        = acc_str[0] ? (float)atof(acc_str) : 0.0f;
        fyGPSValid      = true;
        ...
        httpd_resp_send(req, "{\"error\":\"lat,lon required\"}", 28);
    }
    return ESP_OK;
}

static esp_err_t handler_patterns(httpd_req_t* req) {
    httpd_resp_set_type(req, "application/json");
    resp_chunk(req, "{\"macs\":[");
    for (size_t i = 0; i < sizeof(flock_mac_prefixes)/sizeof(flock_mac_prefixes[0]); i++) {
        if (i > 0) resp_chunk(req, ",");
        resp_chunkf(req, "\"%s\"", flock_mac_prefixes[i]);
    }
    resp_chunk(req, "],\"macs_mfr\":[");
    for (size_t i = 0; i < sizeof(flock_mfr_mac_prefixes)/sizeof(flock_mfr_mac_prefixes[0]); i++) {
        if (i > 0) resp_chunk(req, ",");
        resp_chunkf(req, "\"%s\"", flock_mfr_mac_prefixes[i]);
    }
    resp_chunk(req, "],\"macs_soundthinking\":[");
    for (size_t i = 0; i < sizeof(soundthinking_mac_prefixes)/sizeof(soundthinking_mac_prefixes[0]); i++) {
        if (i > 0) resp_chunk(req, ",");
        resp_chunkf(req, "\"%s\"", soundthinking_mac_prefixes[i]);
    }
    resp_chunk(req, "],\"names\":[");
    for (size_t i = 0; i < sizeof(device_name_patterns)/sizeof(device_name_patterns[0]); i++) {
        if (i > 0) resp_chunk(req, ",");
        resp_chunkf(req, "\"%s\"", device_name_patterns[i]);
    }
    resp_chunk(req, "],\"mfr\":[");
    for (size_t i = 0; i < sizeof(ble_manufacturer_ids)/sizeof(ble_manufacturer_ids[0]); i++) {
        if (i > 0) resp_chunk(req, ",");
        resp_chunkf(req, "%u", ble_manufacturer_ids[i]);
    }
    resp_chunk(req, "],\"raven\":[");
    for (size_t i = 0; i < sizeof(raven_service_uuids)/sizeof(raven_service_uuids[0]); i++) {
        if (i > 0) resp_chunk(req, ",");
        resp_chunkf(req, "\"%s\"", raven_service_uuids[i]);
    }
    resp_chunk(req, "]}");
    httpd_resp_send_chunk(req, NULL, 0);
    return ESP_OK;
...
static esp_err_t handler_export_json(httpd_req_t* req) {
    httpd_resp_set_type(req, "application/json");
...
static esp_err_t handler_export_csv(httpd_req_t* req) {
    httpd_resp_set_type(req, "text/csv");
    httpd_resp_set_hdr(req, "Content-Disposition",
                       "attachment; filename=\"flockyou_detections.csv\"");
    resp_chunk(req, "mac,name,rssi,method,first_seen_ms,last_seen_ms,count,"
                    "is_raven,raven_fw,latitude,longitude,gps_accuracy\n");
    if (fyMutex && xSemaphoreTake(fyMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
        for (int i = 0; i < fyDetCount; i++) {
            FYDetection& d = fyDet[i];
            if (d.hasGPS) {
...
static esp_err_t handler_export_kml(httpd_req_t* req) {
    httpd_resp_set_type(req, "application/vnd.google-earth.kml+xml");
    httpd_resp_set_hdr(req, "Content-Disposition",
                       "attachment; filename=\"flockyou_detections.kml\"");
    writeDetectionsKML(req);
    return ESP_OK;
}

// Stream a SPIFFS file as chunked HTTP response. Returns false if not found.
static bool fyServeSpiffsFile(httpd_req_t* req, const char* path) {
    FILE* f = fopen(path, "r");
    if (!f) return false;
    char buf[512];
    int n;
...
static esp_err_t handler_history(httpd_req_t* req) {
    httpd_resp_set_type(req, "application/json");
    if (!fySpiffsReady || !fyServeSpiffsFile(req, FY_PREV_FILE)) {
        httpd_resp_send(req, "[]", 2);
    }
    return ESP_OK;
}

static esp_err_t handler_history_json(httpd_req_t* req) {
    if (!fySpiffsReady) {
        httpd_resp_set_status(req, "404 Not Found");
        httpd_resp_send(req, "{\"error\":\"no prior session\"}", 28);
        return ESP_OK;
    }
    httpd_resp_set_type(req, "application/json");
    httpd_resp_set_hdr(req, "Content-Disposition",
                       "attachment; filename=\"flockyou_prev_session.json\"");
    if (!fyServeSpiffsFile(req, FY_PREV_FILE)) {
        httpd_resp_set_status(req, "404 Not Found");
        httpd_resp_send(req, "{\"error\":\"no prior session\"}", 28);
    }
    return ESP_OK;
...
static esp_err_t handler_history_kml(httpd_req_t* req) {
    if (!fySpiffsReady) {
        httpd_resp_set_status(req, "404 Not Found");
        httpd_resp_send(req, "{\"error\":\"no prior session\"}", 28);
        return ESP_OK;
    ...
    if (size <= 0) {
        fclose(f);
        httpd_resp_set_status(req, "404 Not Found");
        httpd_resp_send(req, "{\"error\":\"prior session empty\"}", 31);
        return ESP_OK;
    }

    char* content = (char*)malloc(size + 1);
    if (!content) { fclose(f); return ESP_FAIL; }
    ...
    if (!err && doc.is<JsonArray>()) {
        ...
        for (JsonObject d : doc.as<JsonArray>()) {
            ...
            resp_chunkf(req, "<Placemark><name>%s</name>\n", d["mac"] | "?");
            resp_chunkf(req, "<styleUrl>#%s</styleUrl>\n", isRaven ? "raven" : "det");
            resp_chunk(req, "<description><![CDATA[");
            if (d["name"].is<const char*>() && strlen(d["name"] | "") > 0)
                resp_chunkf(req, "<b>Name:</b> %s<br/>", d["name"] | "");
            resp_chunkf(req, "<b>Method:</b> %s<br/><b>RSSI:</b> %d<br/><b>Count:</b> %d",
                d["method"] | "?", d["rssi"] | 0, d["count"] | 1);
            if (isRaven && d["fw"].is<const char*>())
                resp_chunkf(req, "<br/><b>Raven FW:</b> %s", d["fw"] | "");
            resp_chunk(req, "]]></description>\n");
            resp_chunkf(req, "<Point><coordinates>%.8f,%.8f,0</coordinates></Point>\n",
...
static esp_err_t handler_clear(httpd_req_t* req) {
    ...
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, "{\"status\":\"cleared\"}", 20);
...
static void fySetupServer() {
    ...
    config.stack_size        = 16384; // ArduinoJson + KML generation needs headroom

    if (httpd_start(&fyHttpd, &config) != ESP_OK) {
        printf("[FLOCK-YOU] HTTP server start failed\n");
        return;
    }
    ...
    const httpd_uri_t routes[] = {
        ...
        { "/api/detections",  HTTP_GET, handler_detections,   NULL },
        { "/api/stats",       HTTP_GET, handler_stats,        NULL },
        { "/api/gps",         HTTP_GET, handler_gps,          NULL },
        { "/api/patterns",    HTTP_GET, handler_patterns,     NULL },
        { "/api/export/json", HTTP_GET, handler_export_json,  NULL },
        { "/api/export/csv",  HTTP_GET, handler_export_csv,   NULL },
        { "/api/export/kml",  HTTP_GET, handler_export_kml,   NULL },
        { "/api/history",     HTTP_GET, handler_history,      NULL },
        { "/api/history/json",HTTP_GET, handler_history_json, NULL },
        { "/api/history/kml", HTTP_GET, handler_history_kml,  NULL },
        { "/api/clear",       HTTP_GET, handler_clear,        NULL },
    };
    for (const auto& r : routes) {
        httpd_register_uri_handler(fyHttpd, &r);
    }
...
// ============================================================================
// SERIAL MONITOR TASK
// Watches USB CDC stdin for the DeFlock desktop heartbeat.
// Sets fySerialByteReceived so the main loop can act on it from task context.
// ============================================================================

static void fySerialMonitorTask(void* /*pvParam*/) {
    while (1) {
        int c = fgetc(stdin);
        if (c != EOF) {
            fySerialByteReceived = true;
        } else {
...
// ============================================================================
// MAIN LOOP TASK  (equivalent to Arduino loop())
// ============================================================================

static void fyMainTask(void* /*pvParam*/) {
    while (1) {
        // Serial host detection (heartbeat from DeFlock desktop app)
        if (fySerialByteReceived) {
            fySerialByteReceived  = false;
        ...
        // Apply deferred companion mode switch
        ...
        // Auto-save session to SPIFFS every 15s when detections changed
...
// ============================================================================
// ENTRY POINT  (equivalent to Arduino setup())
// ============================================================================
...
extern "C" void app_main() {
    ...
    // Init LEDC for tone generation
    fyLedcInit();

    // Init SPIFFS — promote any leftover session to prev_session before use
    fySpiffsReady = fyInitSpiffs();
    if (fySpiffsReady) fyPromotePrevSession();
    ...
    // BLE GATT server for DeFlock app connectivity
    ...
    // Boot crow calls play while BLE is already scanning
    fyBootBeep();
    ...
    printf("[FLOCK-YOU] Detection: MAC prefix, device name, manufacturer ID, Raven UUID\n");
    printf("[FLOCK-YOU] Ready - BLE GATT + AP mode\n\n");

    // Serial monitor runs in its own task (blocking fgetc on USB CDC stdin)
    xTaskCreate(fySerialMonitorTask, "fy_serial", 2048, NULL, 3, NULL);

    // Main loop
    xTaskCreate(fyMainTask, "fy_main", 8192, NULL, 5, NULL);
}
